 ...
 define('customer360:views/fields/enum-plus', 'views/fields/enum', function (Dep) {
    ...
    return Dep.extend({
        ...
        setupOptions: function () {
            this.wait(
                this.ajaxGetRequest('Account?select=name,type').then(function (response) {
                    response.list.forEach(function (acc) {
                        this.params.options.push(acc.name);
                        this.translatedOptions[acc.name] = acc.type;   
                    }.bind(this));
                }.bind(this))
            );
        },
